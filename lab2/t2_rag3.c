...
ESOS_USER_TASK(traffic_signal) {
  ESOS_TASK_BEGIN();
  while(TRUE) {
    while (SW1 && SW2) {    //SW1 denotes that the stoplight is in "Normal Operation" and SW2 denotes non-turn signal
    ...
    while(SW1 && !SW2) {    // SW1 denotes normal operation and !SW2 denotes Turn signal Active
    red_ns = 1;
    amber_ns = 0;
    green_ns = 0;
    red_ew = 0;
    amber_ew = 0;
    green_ew = 1;
    for (counter = 0; counter < 80; counter++){ // 4 times per second, 10 seconds, 2 cycles
	    green_ew = !green_ew;
        ESOS_TASK_WAIT_TICKS(125); // wait 0.125 seconds
    }
    ...
    amber_ew = 0;
    green_ew = 1;
    ESOS_TASK_WAIT_TICKS(10000); // Wait 10 sec
    ...
    amber_ew = 1;
    green_ew = 0;
    ESOS_TASK_WAIT_TICKS(3000); // wait 3 sec
    ...
    while(!SW1 && SW2) {  // !SW1 denotes that the stoplight is in "Rush-Hour" operation
    ...
    //fourth stage  N-S = green | E-W = red
    red_ns = 0;
    amber_ns = 0;
    green_ns = 1;
    red_ew = 1;
